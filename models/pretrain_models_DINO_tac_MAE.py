...
class VTMAE(nn.Module):
    ...
    def forward(self, x, use_vision=True, use_tactile=True):
        ...
        num_patches = num_tactile_patches#num_image_patches + num_tactile_patches
        ...
        tokens = tactile_tokens          #torch.cat((image_tokens, tactile_tokens), dim=1)
        ...
        image_perc =0.0   # num_image_patches/num_patches
        ...
        masked_indices = masked_indices_tactile #torch.cat((masked_indices_image, masked_indices_tactile), dim=1)
        unmasked_indices = unmasked_indices_tactile #torch.cat((unmasked_indices_image, unmasked_indices_tactile), dim=1)
        ...
        decoder_tokens[:,start_index:end_index] = decoder_tactile_tokens # torch.cat((decoder_image_tokens, decoder_tactile_tokens), dim=1)
...
class MAEExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        obs_viso = obs_viso.reshape(batch_size*3*self.frame_stack,-1,70,70)   # 3:视 ，触，触
