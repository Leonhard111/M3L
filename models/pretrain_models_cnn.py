...
class EarlyCNN(nn.Module):
    def __init__(self, in_channels, encoder_dim, key='image'):
        ...
        self.conv2 = nn.Conv2d(encoder_dim//8, encoder_dim//4, 5, stride=2, padding=0)
        #if key == 'image':
    ...
    def __init__(
        ...
        *,
        ...
        if self.early_conv_masking:
            ...
            self.early_conv_tactile1 = EarlyCNN(self.encoder.tactile_channels, encoder_dim, key='tactile')
            self.early_conv_tactile2 = EarlyCNN(self.encoder.tactile_channels, encoder_dim, key='tactile')
    ...
    def get_embeddings(self, x, eval=True, use_vision=True, use_tactile=True):
        ...
        if self.early_conv_masking:
            ...
            if self.num_tactiles > 0 and use_tactile:
                tactile_tokens_list = []
                #for i in range(1,self.num_tactiles+1):
                tactile_tokens_list.append(self.early_conv_tactile1(x['tactile1']))
                tactile_tokens_list.append(self.early_conv_tactile2(x['tactile2']))
                tactile_tokens = torch.cat(tactile_tokens_list, dim=1)
            else:
                tactile_tokens = torch.zeros((batch, 0, image_tokens.shape[-1])).to(device)
        else:
