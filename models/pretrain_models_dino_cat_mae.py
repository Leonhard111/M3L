...
class MAEExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.Space, 
        ...
        self.vit_layer = VTT(   # 
            image_size = (70, 70), # not used
            tactile_size = (70, 70), # not used
            image_patch_size = 14, # not used
            tactile_patch_size = 14, # not used
            dim = dim_embeddings, 
            depth = 1,
            heads = 4,
        ...
        self.mlp = nn.Sequential(
            nn.Linear(dim_embeddings*2, dim_embeddings * 2),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(dim_embeddings * 2, dim_embeddings * 2),
            nn.ReLU(),
        ...
        self.query = nn.Parameter(torch.randn(1, 1, dim_embeddings))
        
        # 添加查询投影层
        self.query_projection = nn.Linear(dim_embeddings, dim_embeddings)
        self.key_projection = nn.Linear(dim_embeddings, dim_embeddings)
  

    def weighted_pooling(self, tokens):
        """使用可学习的查询向量进行加权平均"""
        ...
        # 投影查询和键
        query = self.query_projection(query)  # [batch_size, 1, dim]
        keys = self.key_projection(tokens)    # [batch_size, seq_len, dim]
        
        # 计算注意力分数
        attention_scores = torch.matmul(query, keys.transpose(-1, -2))  # [batch_size, 1, seq_len]
        attention_scores = attention_scores / math.sqrt(self.dim_embeddings)  # 缩放
        
        # 注意力权重 (softmax归一化)
        attention_weights = F.softmax(attention_scores, dim=-1)  # [batch_size, 1, seq_len]
        
        # 加权求和
        weighted_sum = torch.matmul(attention_weights, tokens)  # [batch_size, 1, dim]
        
        return weighted_sum.squeeze(1)  # [batch_size, dim]
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        #obs_viso = self.dino_model(vt_torch['image'])
        # observations = torch.cat(obs_viso, observations,dim= 1 )
        observations = self.vit_layer.transformer(observations)
        observations = torch.mean(observations, dim=1)
        

        # flattened = self.weighted_pooling(token_features)
        flattened = self.flatten(observations)
        flattened = torch.cat((flattened, obs_viso),dim= -1 )
